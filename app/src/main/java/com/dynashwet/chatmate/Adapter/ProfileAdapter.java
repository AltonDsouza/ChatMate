...
public class ProfileAdapter extends AAH_VideosAdapter {
...
//
    ...
    public ProfileAdapter(List<MyModel> list_urls, Picasso p, Context mCtx) {
        this.list = list_urls;
        this.picasso = p;
        this.mCtx = mCtx;
//        this.fragment = comeFromWhichFragment;
    }
    ...
    public AAH_CustomViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType==TYPE_TEXT) {
            View itemView1 = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.single_text_profile, parent, false);
            return new MyTextViewHolder(itemView1);
        } else {
            View itemView = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.single_card_profile, parent, false);
...
//
            ...
            ((MyTextViewHolder) holder).overflow.setOnClickListener(view-> {
                ...
                if(fragment.equals("ProfileActivity")){
                                            ...
                                            public void onResponse(String response) {
                                                try {
                                                    ...
                                                    if(msg.equals("Success")){
                                                        ...
                                                        Toast.makeText(mCtx, "Unblock user from Menu.", Toast.LENGTH_SHORT).show();
                                                    }
                                                    else if(msg.equals("Error")){
                                                        String reason = jsonObject.getString("reason");
                                                        if(reason.equals("SameUID")){
                                                            Toast.makeText(mCtx, ""+reason, Toast.LENGTH_SHORT).show();
                                                        }
                                                        else {
                                                            Toast.makeText(mCtx, ""+reason, Toast.LENGTH_SHORT).show();
                                                        }
...
//                        like_count.setText(liked + "likes");
            ...
            ((MyTextViewHolder)holder).profile_image.setOnClickListener(view -> {
            ...
            ((MyTextViewHolder)holder).theOneWhoGoesToProfile.setOnClickListener(view -> {
                //UIDcomingFromPublic
...
//            if(fragment.equals("PublicFragment")){
//                if(list.get(position).getUploadby().equals(UID))
//                {
//                    ((MyViewHolder) holder).send_friendReq.setVisibility(View.GONE);//On my post hide friend req button
//                }
//
//                if(list.get(position).getIsFriend().equals("Yes")){
//                    ((MyViewHolder) holder).image_sent_friendReq.setVisibility(View.INVISIBLE);
//                    ((MyViewHolder) holder).send_friendReq.setVisibility(View.GONE);
//                }
//                else if(list.get(position).getIsFriend().equals("No")){
//                    ((MyViewHolder) holder).image_sent_friendReq.setVisibility(View.INVISIBLE);
//                    ((MyViewHolder) holder).send_friendReq.setVisibility(View.VISIBLE);
//                }
//                else {//Req
...
//            }
            ...
            ((MyViewHolder) holder).overflow.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if(fragment.equals("ProfileActivity")){//Current Fragment/Activity
                        popup.getMenu().findItem(R.id.action_deletepost).setVisible(true);
                        popup.getMenu().findItem(R.id.action_report_user).setVisible(false);
                        popup.getMenu().findItem(R.id.action_block_user).setVisible(false);
                    }
...
//                        ((MyViewHolder) holder).likescount.setText(Integer.parseInt(String.valueOf(((MyViewHolder) holder).likescount.getText()))-1);
            ...
            ((MyViewHolder)holder).profile_image.setOnClickListener(view -> {
                Intent i  = new Intent(mCtx.getApplicationContext(), EnlargedView.class);
                i.putExtra("post_image", list.get(position).getPic());
                mCtx.startActivity(i);
            });

            ((MyViewHolder)holder).theOneWhoGoesToProfile.setOnClickListener(view -> {
                //UIDcomingFromPublic
                Intent i  = new Intent(mCtx.getApplicationContext(), ProfileActivity.class);
                i.putExtra("UIDcomingFromPublic", list.get(position).getUploadby());
                mCtx.startActivity(i);
